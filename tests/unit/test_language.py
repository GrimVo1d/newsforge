...
def test_detect_english():
    assert (
        detect_language("The quick brown fox jumps over the lazy dog repeatedly today") == "english"
    )


def test_detect_russian():
    assert (
        detect_language("Российская федерация объявляет о новых возможностях в области технологий")
        == "russian"
    )
